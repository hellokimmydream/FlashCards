...
export default class AuthController {
  ...
  async register({ request, response, session }: HttpContext) {
    ...
    if (existing) {
      session.flash('Erreur', 'Veuillez choisir un autre username')
      return response.redirect().back()
    }

    await User.create({ username, password })
    session.flash('Réussite', 'Compte créé. Vous pouvez vous connecter.')
  ...
  async login({ request, auth, response, session }: HttpContext) {
    ...
    try {
      ...
      session.flash('Réussite', 'Connecté')
      return response.redirect('/decks')
    } catch {
      session.flash('Erreur', 'Login incorrect')
      return response.redirect().back()
    }
  }
  ...
  async logout({ auth, response, session }: HttpContext) {
    ...
    session.flash('Réussite', 'Déconnecté')
    return response.redirect('/login')
  }
}